...
function RiakNode(options) {
    ...
    this._logger = require('./riaklogger')();
    this.executeCount = 0;
    ...
    this._getConnectionFromPool = function (cmd_name) {
        ...
        do {
            ...
            if (conn.closed) {
                 ...
                 */
		this._logger.debug('%s execute of %s attempted on closed connection', this.name, cmd_name);
                conn.executeDone();
                conn = null;
            }
        } while (!conn);
    ...
    this._returnConnectionToPool = function (conn) {
	    ...
	    this._logger.debug("%s _returnConnectionToPool a: %d, cnc: %d",
                this.name, available.length, currentNumConnections);
        } else {
	    this._logger.debug('%s connection returned to pool during shutdown.', this.name);
    ...
    this._decrementConnectionCount = function () {
	    ...
	    this._logger.debug("%s cnc will decrement less than zero! (%d)",
	...
	this._logger.debug("%s expiring idle connections", name);
        var now = Date.now();
        var count = 0;
        var na = [];
        var conn = null;
        ...
        available = na;
	/* jshint validthis:true */
	this._logger.debug("%s expired %d connections.", name, count);
    }
    ...
    this._createNewConnection = function (postConnectFunc, postFailFunc, healthCheck) {
	    ...
	    self._logger.debug("%s conn.on-connected", this.name);
	    ...
	    self._logger.debug("%s conn.on-connectFailed", this.name);
	...
	self._logger.debug("%s initializing health checker", self.name);
	    ...
	    self._logger.debug("%s health check success", self.name);
            self.emit(EVT_SC, self, self.state);
        }

        function hcFailed(err) {
	    self._logger.debug("%s failed health check:", err);
	    ...
	    self._logger.debug("%s running health check %d, delay: %d", self.name, n, d);
        }
    ...
    this._doHealthCheck = function () {
        switch (this.state) {
		    ...
		    this._logger.error('%s already in health check, external load balancer:',
                        this.name, this.externalLoadBalancer);
                } else {
		    this._logger.debug("%s already in health check", this.name);
                }
                break;
            case State.RUNNING:
                if (this.externalLoadBalancer) {
		    this._logger.warn('%s would have health checked, external load balancer:',
                        this.name, this.externalLoadBalancer);
                } else {
                ...
                break;
            default:
		this._logger.warn('%s health check attempted in state:',
    ...
    function runShutdown(self, callback) {
	    ...
	    self._logger.debug("%s closing %d connections",
	    ...
	    self._logger.debug("%s closed connections (cnc: %d)",
                self.name, currentNumConnections);
        }
	    ...
	    self._logger.debug("%s shut down.", self.name);
            if (self.executeCount !== 0) {
		self._logger.warn('%s execution count (%d) NOT ZERO at shutdown',
                    self.name, self.executeCount);
            }
            if (currentNumConnections !== 0) {
		self._logger.warn('%s connection count (%d) NOT ZERO at shutdown',
                    self.name, currentNumConnections);
            }
            self.emit(EVT_SC, self, self.state);
		...
		self._logger.debug("%s connections still not closed", self.name);
                sdb.backoff();
            }
        }

        function sdBackoffHandler(n, d) {
	    self._logger.debug("%s running shutdown %d, delay: %d", self.name, n, d);
        }

        function sdFailHandler() {
	    self._logger.warn("%s shutting down after max tries", self.name);
            nodeShutdown();
        }
...
RiakNode.prototype.start = function(callback) {
    ...
    this._logger.debug('%s starting', this.name);
    ...
    async.parallel(funcs, function (err, rslts) {
        if (err) {
	    self._logger.error('%s (%d) error during start:', self.name, self.executeCount, err);
        }
...
RiakNode.prototype.stop = function(callback) {
    ...
    this._logger.debug("%s shutting down", this.name);
    this.emit(EVT_SC, this, this.state);
...
RiakNode.prototype.execute = function (command) {
    ...
    this._logger.debug("%s executing command '%s'", this.name, command.name);
    ...
    if (this._executeAllowed()) {
        ...
        if (!conn) {
		    ...
		    self._logger.debug("%s executing command '%s' (new connection %d)",
                    ...
                    if (self._executeAllowed() && newConn.execute(command)) {
                        self.executeCount++;
			self._logger.debug("%s (%d) executed command: '%s'",
                            self.name, self.executeCount, command.name);
                    }
                }, function (err) {
		    self._logger.debug("%s (%d) command execution failed '%s'",
                        self.name, self.executeCount, command.name);
                    self._doHealthCheck();
		...
		this._logger.debug('%s all connections in use and at max', this.name);
                executed = false;
            }
        } else {
	    this._logger.debug("%s executing command '%s' (existing connection %d)",
                this.name, command.name, this.executeCount);
            if (conn.execute(command)) {
                this.executeCount++;
		this._logger.debug("%s (%d) executed command: '%s'",
...
RiakNode.prototype._responseReceived = function (conn, command, code, decoded) {
    ...
    this._logger.debug("%s command '%s' _responseReceived: %d", this.name, command.name, code);
    var self = this;
    ...
    function onSuccess() {
        ...
        if ((hasDone && decoded.done) || !hasDone) {
            self.executeCount--;
            self._returnConnectionToPool(conn);
	    self._logger.debug('%s command %s complete (%d)',
                self.name, command.name, self.executeCount);
        }
...
RiakNode.prototype._connectionClosed = function (conn) {
    ...
    this._logger.debug("%s connection closed; command: '%s', in-flight: '%s'",
...
RiakNode.prototype._maybeRetryCommand = function (command, errfunc) {
    ...
    this._logger.debug("%s command %s remaining tries %d -> %d",
...
Builder.prototype = {
    ...
    /**
     * Pass a winston logger to RiakNode. If none define, then logging to
     * console.
     */
    withLogger: function(logger) {
	if (logger !== undefined) {
	    this._logger = require('./riaklogger')(logger);
	}
	return this;
    },
    /**
